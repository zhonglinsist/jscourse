...
var myWaterFull = {
    ...
    start:function(){
        ...
        //var n = Math.floor(document.documentElement.offsetWidth/li_W);//向下取整//文艺青年写法
        ...
        for(var i =0;i<len;i++){
            ...
            if(i < self.colNum){//self.colNum是列数，所以小于它就是第一行了
                ...
                var curCol = Math.min.apply(null,self.heightArray);//取得数组中的最小值，区块中高度值最小的那个
                minKey = myUtil.getKey(self.heightArray,curCol);//获取最小高度列的列数
                myUtil.setStyle(item,'top',curCol);
